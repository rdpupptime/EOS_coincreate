...
function pushAction() {
	try {
		...
		tp.pushEosAction(params).then(function (data) {
			var result = JSON.stringify(JSON.parse(JSON.stringify(data)), null, 2);
			$('.consoleLog').html(result);
		});
	} catch (e) {
...
function pushAction1() {
	try {
		var str = '{"actions": [{"account": "eosio.token","name": "transfer","authorization": [{"actor": "' + $.trim($('#inp1').val()) + '","permission": "active"}],"data": {"from": "' + $.trim($('#inp1').val()) + '","to": "emmmmmmmmmmm","quantity": "3.0000 EOS","memo": "' + '1-' + $.trim($('#inp1').val()) + '-' + $.trim($('#inp2').val()) + '-' + $.trim($('#inp3').val()) + '"}}]}';
...
function getTable1() {
	try {
		...
		var params = JSON.parse(str);
		tp.getTableRows(params).then(function (data) {
			var result = JSON.stringify(JSON.parse(JSON.stringify(data)), null, 2);
			$('.consoleLog').html(result);
		});
	} catch (e) {
